...
npm run fetch:calendar   # refresh bundled NYSE calendar from Alpaca
```
...
- Money is **integer cents** (`Cents` branded type), never floats. Percentages
  ...
  helper in `packages/engine/src/core/money.ts` (half away from zero).
...
`packages/engine/src/core` has **zero dependencies and no side effects**: no Node APIs,
